...
def rate_locations(self, locations, global_time, prompt_meta, recent_impressions, nearby_situations):
    ...
    for location in locations.locations.values():
        prompt = rate_location_prompt.format(self.name, self.hourly_plans, global_time, locations.get_location(self.location), self.description, recent_impressions, nearby_situations, location.name)
