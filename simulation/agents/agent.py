...
class Agent:
    ...
    def daily_planning(self, global_time, prompt_meta, recent_impressions, newthings):
        ...
        """
        system = agent_plan_system.format(self.name, self.description, '\n'.join(recent_impressions), '\n'.join(newthings))
        global_hour = global_time.split(':')[0]
        prompt = agent_plan_prompt.format(str(global_hour))
        self.daily_plans = GPT_request(system, prompt_meta.format(prompt), gpt_parameter={"max_tokens": 1000})
    ...
    def hourly_planning(self, 
        ...
        system = hourly_planning_system.format(self.name, self.description, '\n'.join(recent_impressions), '\n'.join(newthings), self.daily_plans)
        ...
        self.hourly_plans = GPT_request(system, prompt_meta.format(prompt), gpt_parameter={"max_tokens": 50})
        experience = self.memory_daily_plans(global_time)
        return experience
    ...
    def execute_action(self, global_time, prompt_meta, recent_impressions, nearby_situations):
        ...
        """
        system = agent_execute_action_system.format(self.name, self.description, '\n'.join(recent_impressions), self.daily_plans)
        ...
        self.action = GPT_request(system, prompt_meta.format(prompt), gpt_parameter={"max_tokens": 120})
        return self.action