...
for repeat in range(repeats):
    ...
    if new_day or repeat == 0:
        for agent in agents:
            experience =agent.daily_planning(global_time, prompt_meta, ['待修改'], memory.get_newthings_str(agent.name, memory_limit))
    ...
    if new_hour:
        for agent in agents:
            experience =agent.hourly_planning(agents, locations.get_location(agent.location), global_time, town_areas, prompt_meta, ['待修改'], memory.get_newthings_str(agent.name, memory_limit))
            memory.add_experience(experience, 'plan')
            if log_actions:
    ...
    for agent in agents:
        # Execute the action
        action = agent.execute_action(global_time, prompt_meta, ['待修改'], memory.get_newthings_str(agent.name, memory_limit))
        priority = agent.rate_experience(locations, global_time, prompt_meta, ['待修改'], memory.get_newthings_str(agent.name, memory_limit), action)
    ...
    if new_hour and repeat != 0:
        for agent in agents:
            place_ratings = agent.rate_locations(locations, global_time, prompt_meta, ['待修改'], memory.get_newthings_str(agent.name, memory_limit))
            if log_ratings:
